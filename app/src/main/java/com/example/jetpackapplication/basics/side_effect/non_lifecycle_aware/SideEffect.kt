package com.example.jetpackapplication.basics.side_effect.non_lifecycle_aware
...
/*
  It is not life cycle aware
  It runs on UI thread every time composable runs so do not render UI inside this
 */
...
fun MySideEffect() {
	...
	SideEffect {
		val counterState = when(counter){
			0 -> CounterState.NOTSTARTED
			10 -> CounterState.COMPLETED
			else -> CounterState.INPROGRESS
		}
		Log.i("MySideEffect" , "Counter state changes : : $counterState")
	}
...
@Composable
fun DefaultMySideEffectStatePreview() {
	MySideEffect()
}